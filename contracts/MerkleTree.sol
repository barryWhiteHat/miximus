contract MerkleTree {
    ...
    function insert(Mtree MT, bytes32 com) internal returns (bool res) {
        require (MT.cur != no_leaves - 1);
        MT.leaves2[0][MT.cur] = com;
        updateTree(MT); // TODO: this is expensive
        leafAdded(MT.cur);
        MT.cur++;
    ...
    function getMerkleProof(Mtree MT, uint index) constant returns (bytes32[29], uint[29]) {
        ...
        for (uint i=0 ; i < tree_depth; i++) {
            ...
            if (index%2 == 0) {
                MerkleProof[i] = getUniqueLeaf(MT, MT.leaves2[i][index + 1],i);
            }
            else {
                MerkleProof[i] = getUniqueLeaf(MT, MT.leaves2[i][index - 1],i);
            }
    ...
    function getUniqueLeaf(Mtree MT, bytes32 leaf, uint depth) returns (bytes32) {
        if (leaf == 0x0) {
    ...
    function updateTree(Mtree MT) internal returns(bytes32 root) {
        ...
        for (uint i=0 ; i < tree_depth; i++) {
            ...
            if (CurrentIndex%2 == 0) {
                ...
                leaf2 = getUniqueLeaf(MT, MT.leaves2[i][CurrentIndex + 1], i);
            } else {
                leaf1 = getUniqueLeaf(MT, MT.leaves2[i][CurrentIndex - 1], i);
    ...
    function getRoot(Mtree MT) constant returns(bytes32 root) {
